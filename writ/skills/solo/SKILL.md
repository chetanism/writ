...
You are setting up a project's development process. The output is a `writ/` tree, a CI gate, and a
...
> code, stop and use **`/writ:team`** instead — it designs the handoffs, and retrofitting them
...
**One exception, and it is not a document: `.writ-interview.md` at the target root.** Append a
...
- **Look for `.writ-interview.md` at the target first.** A previous run left it there if one got
  ...
  rule, including what to do when a `<writ>/spec/` exists as well. With no such file, say nothing
  and carry on.
...
- **Ask what to call the tree.** Default `writ/` — the authoritative body of documents an agent
...
- **Check the fourteen skill names against what is already there.** The kit emits `slice-open`,
  ...
  it is the project's or the user's — and ask once, in one `AskUserQuestion`, with two options:
    1. **Prefix every kit skill with `writ-`** (the default): `/writ-cleanup`, `/writ-slice-open`
...
1. `writ/spec/BRD.md` — the case for the product, in prose. No identifier tables in it.
2. The registers, one file each, from the interview: the requirement areas as
   `writ/spec/requirements/<AREA>/index.md` from `writ/process/templates/requirement-area.md`,
...
3. `writ/spec/ID-REGISTRY.md` — a row for every family you just used, with its width, and a
   directory owner for the requirement areas. Mark traceable exactly those a slice can claim and a
   test can prove.
4. `writ/spec/MILESTONE-PLAN.md` — purpose, gates, phases as `P01`, `P02` with names, foundation
   specs, what is not built, external tracks. Risks and questions went into their registers.
5. `writ/spec/<AREA>-SPEC.md` — a foundation spec for each shape everything else inherits (the
...
6. `writ/spec/CHANGELOG.md` — the seed row, `X-001`, dated today, touching `v0.1`.
...
- Give each one a work order at `writ/process/work-orders/<milestone>/<phase>/<NNN>.md`, named
  for its number, with front matter from `writ/process/templates/work-order.md`. The milestone
...
`/writ:adopt` needs the other end of it, and a rule with no perimeter cannot be turned on for one
...
**If phase 0 chose a name other than `writ`, rewrite the emitted tree now**, before anything else
reads it: move `writ/` to `<name>/`, then replace every `writ/` path — the trailing slash is what
...
itself reads every path from the config, so `ledger.py` needs nothing. Then grep for `writ/` and
...
writ/spec/requirements/README.md   the whole detail process, in one file
writ/qa/README.md                     the whole scenario process, in one file
writ/process/templates/requirement-detail.md
writ/process/templates/test-scenarios.md
```
...
**Solo, the scenario track waits for a second pair of hands, and `writ/qa/README.md`'s owner line
...
```bash
rm -f .writ-interview.md                       # its content is in the documents now
