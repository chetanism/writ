...
with coding agents. The output is a `writ/` tree, a CI gate, a traceability tool, and — the part
that is only in this skill — **a pipeline with named roles and explicit handoffs**.

> **If exactly one human will make every decision, stop and use `/writ:solo`.** It is the same
...
before phase 6 — with one exception, `.writ-interview.md` at the target root, appended after every
...
|---|---|---|
| **BRD owner** | The registers under `writ/spec/`, and `BRD.md` — scope, targets, and every change request | Who decides what is in and out? |
...
Exactly as `/writ:solo` phase 3. Load `references/02-security.md`, pick the domain profile, ask
...
Exactly as `/writ:solo` phase 4. Load `references/03-reliability.md`. Refuse to invent a number
the user did not give you; record it as an open question with a decider instead.

### Phase 5 — Stack and gate roles

Exactly as `/writ:solo` phase 5 — fill every gate role with a concrete command, then load one
...
**One addition:** every document's front matter names its **owner**, and `writ/spec/` gets a
...
- `DEVELOPMENT-PROCESS.md` gains the roles table, the handoff contract, and definition-of-done rows
  ...
  implementer*.
- `.github/CODEOWNERS` — `writ/spec/` to the BRD owner, `writ/decisions/` to whoever approves
  invariants, `writ/process/` to the slicer.

**And the two questions phase 8 asks**, as solo, in one call — attribution is not asked; it is
forbidden, as solo says. First: does `CLAUDE.md` carry the *Talking to me* section
...
`references/07-authoring-style.md` carries the section and the argument. Second, the tracker — an
...
- **`.github/CODEOWNERS` gains `writ/maintenance/`** — to whoever owns the security backlog,
...
- **`.github/CODEOWNERS` gains `writ/spec/requirements/`** — to the approver, **on a line after
  the `writ/spec/` one**: the last matching pattern wins, so a requirements rule placed above the
...
- **Name the test manager**, in `writ/qa/README.md`'s owner line and in `DEVELOPMENT-PROCESS.md`'s
  role table, and give `writ/qa/` to them in CODEOWNERS. Which scenarios are worth a session is a
...
As solo: `.writ-interview.md` deleted, `dev` created or checked out, the whole tree committed
