...
"""The kit's own templates, bootstrapped into a project and held to the kit's own check.
    ...
    python3 writ/tests/test_templates.py
...
def bootstrap(root: str) -> None:
    ...
    for area, (name, rows) in AREAS.items():
        directory = os.path.join(root, "writ", "spec", "requirements", area)
...
class BootstrappedTemplatesTest(unittest.TestCase):
    ...
    def test_the_area_register_is_not_read_as_a_detail_file(self):
        ...
        details = ledger.parse_details(self.root, "writ/spec/requirements")
        self.assertEqual([], [d.path for d in details])

    def test_the_generated_index_lists_the_declared_requirements(self):
        run(self.root, "all")
        index = read(os.path.join(self.root, "writ", "INDEX.md"))
...
class ShippedDocumentsTest(unittest.TestCase):
    ...
    def setUp(self):
        registry = read(os.path.join(TEMPLATES, "writ", "spec", "ID-REGISTRY.md"))
    ...
    def test_every_identifier_the_templates_declare_fits_its_families_pattern(self):
        ...
        wrong = []
        for here, _dirs, files in os.walk(os.path.join(TEMPLATES, "writ")):
            for name in files:
    ...
    def test_every_family_the_registry_declares_owns_something_that_exists(self):
        ...
        for fam in self.families:
            for candidate in (
                os.path.join(TEMPLATES, "writ", fam.owner),
...
class BranchCheckTest(unittest.TestCase):
    ...
    def test_a_detail_file_is_still_claimed_by_the_branch_rule(self):
        self.assertEqual(
            ...
            self.filter(["writ/spec/requirements/FR-ACC/FR-ACC-01.md"]),
        )
    ...
    def test_an_area_register_is_not(self):
        ...
        self.assertEqual(
            ...
            self.filter(
                [
                    "writ/spec/requirements/README.md",
                    "writ/spec/requirements/FR-ACC/index.md",
                    "writ/spec/requirements/NFR-OBS/index.md",
                ]
            ),
        )
...
class PerimeterCheckTest(unittest.TestCase):
    ...
    def test_a_slice_branch_with_its_work_order_passes(self):
        code, out = self.verdict(
            ...
            orders="writ/process/work-orders/m1-launch/P02/042.md",
        )
        self.assertEqual(code, 0, out)

    def test_the_writ_tree_is_outside_the_perimeter_whatever_the_patterns_say(self):
        """A correction to the process must not need a slice of its own, or it is never made."""
        code, out = self.verdict(
            ["**"],
            ["writ/spec/debt.md", "scripts/ledger.config.json", "CLAUDE.md", ".github/workflows/gate.yml"],
        )
        self.assertEqual(code, 0, out)
...
class AdoptedTreeTest(unittest.TestCase):
    """The shipped templates driven the way `/writ:adopt` drives them.
    ...
    def setUpClass(cls):
        ...
        area = os.path.join(cls.root, "writ", "spec", "requirements", "FR-ACC")
        ...
        with open(os.path.join(cls.root, "writ", "spec", "debt.md"), "w") as handle:
            handle.write(
        ...
        orders = os.path.join(cls.root, "writ", "process", "work-orders", "m1", "P01")
    ...
    def test_the_debt_register_declares_its_family(self):
        ...
        self.ledger("all")
        with open(os.path.join(self.root, "writ", "INDEX.md")) as handle:
