# `writ` — an agent-first development process, as a plugin
...
```bash
claude plugin marketplace add chetanism/writ    # once
...
```bash
gh repo clone chetanism/writ ~/src/writ    # the plugin is the repository's own writ/ directory
...
- **A new project, one person building it** → `/writ:solo`.
- **A new project, more than one person** → `/writ:team`. It designs the roles and handoffs, and
  retrofitting those is harder than choosing correctly at the start.
- **A codebase that already exists** → `/writ:adopt`. Different problem, so a different skill
...
**The tree is called `writ/` by default** — the authoritative body of documents an agent reads
...
```
writ/INDEX.md                        generated: every identifier, where it is, what state it is in
writ/spec/README.md                  which file holds which kind of thing, and the rules
writ/spec/BRD.md                     why — the case for the product, in prose; declares nothing
writ/spec/requirements/<AREA>/index.md   what — one register per area, beside its detail files
writ/spec/invariants.md … questions.md   the other registers: one table each, Since and Status columns
writ/spec/CHANGELOG.md               one line per amendment, whatever it amended
writ/spec/changes/README.md          change requests — the unit of post-launch change; ships empty
writ/spec/ID-REGISTRY.md             every identifier family, with its width and owner
writ/spec/MILESTONE-PLAN.md          what this milestone builds; the gates; the phases as P01, P02, …
writ/spec/<AREA>-SPEC.md             the shapes everything inherits
writ/spec/requirements/README.md     the parallel detail track; no detail file ships
...
| Path | What it is |
|---|---|
| `skills/solo/SKILL.md` | `/writ:solo` — ten-phase interview for one developer plus agents |
...
```bash
python3 writ/tests/test_templates.py   # from the repository that carries the plugin
```
...
- **Less of it** — every part has a switch and `writ/process/DEVELOPMENT-PROCESS.md` §15 is the
