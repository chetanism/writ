...
- **The state of the parallel tracks for every requirement the slice will claim** — its detail
  file under `writ/spec/requirements/<area>/` (none, `draft` or `reviewed`) and its scenarios
  under `writ/qa/scenarios/<area>/` (the same). None of it gates the slice: the tracks run beside
