...
4. Ignore generated files, build artefacts, lockfiles and vendored dependencies:
   `<GENERATED ARTEFACTS>`.
5. **Read `writ/maintenance/cleanup-backlog.md` before you start.** It has two tables and
...
- **`writ/decisions/`, `writ/process/work-orders/` and `writ/process/slices/` are immutable by
...
3. **Reconcile `writ/maintenance/cleanup-backlog.md` in the same commits.**
