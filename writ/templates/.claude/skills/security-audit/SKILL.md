...
1. Find the last security review commit:
    ...
    - **A match is only a baseline if that commit actually carried an audit.** A commit that merely
      *discusses* the marker — this prompt's own introduction does — matches the grep and yields an
      empty diff. Confirm the candidate added a report under `writ/maintenance/audits/`; if
...
3. **Read `writ/maintenance/security-backlog.md`, and add everything still open to the
...
`writ/maintenance/security-backlog.md` is the **live** list; a report is a point-in-time
