...
**Everything `/writ:solo` does, plus the part that only exists here: a pipeline with named roles,
...
[`/writ:solo`](solo.md), not this.** Team mode is about contention, and contention needs a crowd;
...
claiming work and CODEOWNERS, argued out. [`/writ:adopt`](adopt.md) if the codebase already
