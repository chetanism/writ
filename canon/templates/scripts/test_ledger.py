...
class LedgerTest(unittest.TestCase):
    ...
    def test_the_proof_column_carries_the_test_name_not_the_line(self):
        self.green()
        self.assertIn("`[FR-ACC-01] signs a person up`", self.fx.read("canon/process/COVERAGE.md"))

    def test_output_is_byte_identical_across_runs(self):
        self.fx.run("all")
        first = self.fx.read("canon/process/COVERAGE.md")
        self.fx.run("all")
    ...
    def test_a_traceable_family_with_no_identifiers_is_fatal(self):
        self.fx.write("canon/spec/BRD.md", BRD.replace("## 2. Invariants", "## 2. Invariance"))
        code, err = self.fx.run("check")
        self.assertEqual(code, 1)
    ...
    def test_an_identifier_declared_twice_in_one_section_is_fatal(self):
        self.fx.write("canon/spec/BRD.md", BRD.replace("| FR-ACC-03 |", "| FR-ACC-01 |"))
        code, err = self.fx.run("check")
        self.assertEqual(code, 1)
    ...
    def test_a_stale_artefact_fails_the_check(self):
        self.fx.run("all")
        self.fx.write("canon/process/COVERAGE.md", "# hand-edited\n")
        code, err = self.fx.run("check")
        self.assertEqual(code, 1)
    ...
    def test_the_queue_names_an_owner_in_team_mode(self):
        ...
        self.assertIn("| ada |", self.fx.read("canon/process/SLICE-QUEUE.md"))
    ...
    def test_a_reviewed_file_with_an_approver_passes_and_counts_as_reviewed(self):
        ...
        self.green()
        self.assertIn("| reviewed | 1 |", self.fx.read("canon/process/COVERAGE.md"))
    ...
    def test_a_requirement_whose_row_carries_emphasis_can_still_be_quoted_verbatim(self):
        ...
        self.detail_on()
        self.fx.write("canon/spec/BRD.md", BRD.replace("| FR-ACC-01 | Sign up. |", "| FR-ACC-01 | Sign up **now**. |"))
    ...
    def test_the_track_is_silent_when_it_is_not_installed(self):
        ...
        self.green()
        self.assertNotIn("## Requirement detail", self.fx.read("canon/process/COVERAGE.md"))
...
class ScenarioTrackTest(unittest.TestCase):
    ...
    def write(self, body=None, **kw):
        self.fx.write("canon/qa/scenarios/FR-ACC/FR-ACC-01.md", scenarios_md(body=body, **kw))
    ...
    def test_a_file_in_the_wrong_area_directory_fails(self):
        self.fx.write("canon/qa/scenarios/FR-BIL/FR-ACC-01.md", scenarios_md())
    ...
    def test_a_project_with_no_scenario_directory_configured_ignores_the_track(self):
        ...
        code, err = self.check()
        self.assertEqual(code, 0, err)
        self.assertNotIn("## Manual test scenarios", self.fx.read("canon/process/COVERAGE.md"))

    def test_the_directory_readme_is_not_read_as_a_scenarios_file(self):
        self.fx.write("canon/qa/scenarios/README.md", "# Scenarios\n\nProse, no front matter.\n")
        code, err = self.check()
        self.assertEqual(code, 0, err)
