...
`canon/process/DEVELOPMENT-PROCESS.md` is this team's document, not a library's. It was written to
...
- **A choice between alternatives with no rule behind it** — that is an ADR in `canon/decisions/`.
...
| Read | For |
|---|---|
| `canon/process/DEVELOPMENT-PROCESS.md`, **the section that states the rule, and §15** | What the rule actually says, and what has already been turned down and why |
...
edit   canon/process/DEVELOPMENT-PROCESS.md  §2.1 the tiers · §15 the record
...
|---|---|---|
| **The declaration rule** | Every tool, check and generated file reads it; removing it is a rewrite, not a setting | It bends by adding rows to `canon/spec/ID-REGISTRY.md` — new families, widths and owners, all Markdown. Ask what they actually need to declare |
