...
| Why one option was chosen over another | an ADR in `canon/decisions/` — **and the line here points at it by number** |
| A rule about how we build — the loop, the gate, sizing, review | `canon/process/DEVELOPMENT-PROCESS.md` |
| A shape everything inherits — an error envelope, an id scheme, a layering rule | the foundation spec `canon/spec/<AREA>-SPEC.md` that owns that area |
