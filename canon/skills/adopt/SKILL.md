...
enough that somebody now depends on it. That is why rows carry `Provenance`, why `canon/spec/debt.md`
...
as test names, every slice demonstrated by hand — is exactly as `/canon:solo` describes it. Read
...
**`.canon-interview.md` at the target root** works exactly as it does in `/canon:solo`: append a
...
- **Look for `.canon-interview.md`** and offer to resume. As `/canon:solo` phase 0.
...
5. **What is wrong.** Straight out: what in here do you know is bad? Write every answer into
   `canon/spec/debt.md`. **Ask this before you ask what the requirements are** — a user who has
...
Load `references/02-security.md` and identify the domain profile, as `/canon:solo` does. Two
...
Ask the stack, then fill every gate role — format, static analysis, types, unit, integration,
contract, traceability — as `/canon:solo` phase 5 does.
...
1. `canon/spec/debt.md` — every answer from phase 2's fifth question, one row each. **Fill this
...
2. `canon/spec/BRD.md` — the case for the product, in prose, in the past tense where it has to be.
   It declares nothing. Where you do not know why something was built, say so rather than inventing
   a rationale.
3. The area registers, `canon/spec/requirements/<AREA>/index.md`, **carrying a `Provenance`
...
6. `canon/spec/ID-REGISTRY.md` — a row per family used, including `DEBT`.
7. `canon/spec/MILESTONE-PLAN.md` — what the *next* milestone builds. Not a retrospective plan for
   what already exists.
8. `canon/spec/CHANGELOG.md` — the seed row, dated today, touching `v0`.
...
Load `references/04-slicing.md` and ask for the slicing criteria, as `/canon:solo` does. Then cut
...
**Three questions here, in one `AskUserQuestion`**, and they are the same three `/canon:solo`
...
Load `references/09-standing-skills.md` and proceed as `/canon:solo` phase 9. One difference in the
...
`scripts/ledger.config.json` as `/canon:solo` phase 10 describes, with
...
```bash
rm -f .canon-interview.md
git checkout -b docs/adopt-canon
git add -A
...
Then the table of what is not installed and what each is waiting for, as `/canon:solo` does — and
