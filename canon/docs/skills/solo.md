...
registers, identifier registry, dependency-ordered slice queue, CI gate, coverage ledger and thirteen
...
| **Produces** | A `canon/` tree, `CLAUDE.md`, `.github/workflows/`, `scripts/`, thirteen skills — committed on `dev` |
...
|---|---|
| 0 | The tree's name, where things go, and whether any of the thirteen skill names collides with one you already have |
...
becomes a row in `canon/spec/questions.md` with an owner. That is a much better outcome than a
...
**It can be resumed.** `.canon-interview.md` is appended after every phase, offered back at phase 0
...
[`/canon:team`](team.md) if more than one person will build it — the roles and handoffs are far
harder to retrofit than to choose. [`/canon:adopt`](adopt.md) if the code already exists.
