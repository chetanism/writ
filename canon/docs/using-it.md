...
**`/canon:solo` for a new project one person is building, `/canon:team` for a new project several
people are building, `/canon:adopt` for a codebase that already exists.** Pick once; they are not
...
any of them: an unanswered question becomes a row in `canon/spec/questions.md` with an owner, which
...
`.canon-interview.md` is appended after every phase, found at phase 0 next time, and deleted at the
commit.

### What you get at the end

**A `canon/` tree, a CI gate, a traceability tool, thirteen skills tuned to your answers, and a
commit on `dev` — or, for an adoption, on `docs/adopt-canon` with every rule switched off.**

The full listing is in [the reference](../README.md#what-you-get). The parts to know on day one:

- `canon/spec/` — what the product is. One kind of thing per file: a narrative BRD that declares
  nothing, then a register per kind of declaration with `Since` and `Status` columns.
- `canon/process/` — how work is done. `DEVELOPMENT-PROCESS.md` is the one document to read; the
...
[§4.1 of the process document](../templates/canon/process/DEVELOPMENT-PROCESS.md) — *the checkable
...
**Point the agent at `canon/` before every non-trivial task and it will plan against the
...
`canon/spec/invariants.md` and the detail file for FR-ACC-01 before you plan this* — because a
