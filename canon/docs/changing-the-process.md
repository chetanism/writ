...
1. **The size tiers are wrong for your codebase.** They are a guess until you have a dozen slices.
...
| `canon/process/DEVELOPMENT-PROCESS.md` | The rule as written for people — the loop, the definition of done, the cadence, §15 |
...
| `canon/spec/ID-REGISTRY.md` | Identifier families — the declaration rule bends here and nowhere else |
| `canon/process/templates/` | The shape of a work order, summary, detail file or scenario file |

### The prompt shape that works

```
Read canon/process/DEVELOPMENT-PROCESS.md and scripts/ledger.config.json first.
...
- **Stating the outcome, not the edit.** *I want smaller slices* gets you a better answer than
  *change `size_budget.S` to 100*, because the agent can tell you that the tiers are also read by
  `stats` and that recalibrating them retroactively makes your historical estimate-accuracy numbers
  meaningless.
...
| No size budget | `size_budget: {}` | §2.1's tiers stop being checkable **and stop being recalibratable** | *Stop enforcing slice sizes.* |
...
| You want | Where | Say |
|---|---|---|
| **Different slice sizes** | `size_budget` in the config, §2.1 in the process doc | *Run `ledger.py stats`, tell me what our tiers should actually be from the last twenty slices, and recalibrate — and say what that does to the historical estimate accuracy.* |
...
| **A new identifier family** | One row in `canon/spec/ID-REGISTRY.md` | *We need a family for performance budgets, `PERF-NNN`, declared in a new register. Add the registry row, create the register and tell me what the tool now expects.* |
...
| **A renamed tree or skill** | Phase 0 chose it; a `git mv` plus a substitution changes it | *Rename `canon/` to `spec/` everywhere, including the ledger config and `CLAUDE.md`.* |
...
It bends by adding rows to `canon/spec/ID-REGISTRY.md` — new families, new widths, new owners, all
