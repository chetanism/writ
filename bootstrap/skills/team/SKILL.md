...
As solo: establish the target directory, refuse to overwrite an existing `docs/spec/`, confirm the
project name. Additionally, ask **how many people** will build this and whether they are in one
timezone. Both change the answers in phase 3.
...
|---|---|---|
| **BRD owner** | `docs/spec/BRD.md`, scope, priority | Who decides what is in and out? |
...
**One addition:** every document's front matter names its **owner**, and `docs/spec/` gets a
...
emit `/maintenance`, `/manual-test` and `docs/process/maintenance/`. Then three things that only
...
- **`.github/CODEOWNERS` gains `docs/process/maintenance/`** — to whoever owns the security backlog,
