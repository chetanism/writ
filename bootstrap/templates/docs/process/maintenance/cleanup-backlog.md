...
| **CL-S2** | Prose in `docs/decisions/`, `docs/process/work-orders/` and `docs/process/slices/` that contradicts the code as it stands now | **Immutable by status.** An ADR records why a decision was made *then*; a work order and a slice summary record what was agreed and what happened. A later fact does not make them wrong, it makes them history. Correcting them destroys the record. A decision that has been superseded gets a **new** ADR | at bootstrap |
