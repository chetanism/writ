...
`docs/spec/BRD.md` is the source of truth for scope, requirements and constraints. Read it before
...
| Document | Role |
|---|---|
| `docs/spec/BRD.md` | **Source of truth.** What we build and why |
| `docs/spec/ID-REGISTRY.md` | **Every identifier family** — owner, declaring section, traceability |
| `docs/spec/MILESTONE-PLAN.md` | What structure this milestone builds; the gates; the amendment log |
| `docs/spec/<AREA>-SPEC.md` | Foundation specs — the shapes everything inherits |
| `docs/process/DEVELOPMENT-PROCESS.md` | **How we work.** The slice loop, the definition of done, traceability |
| `docs/process/SLICE-QUEUE.md` | **In what order.** The queue table is generated from work-order front matter |
| `docs/process/MANUAL-REGRESSION.md` | Standing by-hand scenarios, promoted from demos. Kept short by deletion |
...
Every identifier is prefixed, and every family is declared in `docs/spec/ID-REGISTRY.md` with its
...
<From `docs/spec/BRD.md` §7.2. Violating one is a defect, not a style preference. Name the layer
that enforces each.>
