...
This is one of the three standing prompts `SKILL.md` runs; it owns *what* changes, and `SKILL.md`
owns the branch, the commits, the pull request and the merge.
...
2. Written documents live in `docs/`: the BRD, the foundation specs, ADRs, work orders and slice
...
2. **If a commit is found:**
    - Inspect changes since then: `git diff <commit>..HEAD --stat`, then examine the changed source
      files, schemas, migrations and documents under `docs/`.
...
`SKILL.md` owns the branch, the push, the pull request and the merge. Two obligations are yours
alone:
