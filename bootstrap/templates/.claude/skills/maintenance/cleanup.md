...
4. Ignore generated files, build artefacts, lockfiles and vendored dependencies:
   `<GENERATED ARTEFACTS>`.
5. **Read `docs/process/maintenance/cleanup-backlog.md` before you start.** It has two tables and
...
- **`docs/decisions/`, `docs/process/work-orders/` and `docs/process/slices/` are immutable by
...
3. **Reconcile `docs/process/maintenance/cleanup-backlog.md` in the same commits.**
