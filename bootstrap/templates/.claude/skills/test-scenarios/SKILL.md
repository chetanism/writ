...
The track described in `docs/qa/README.md`. **The detail file already settled what the requirement
...
- **Not declared in the specification, or outside the covered families** — stop and say which.
  ...
  is struck through in `docs/spec/BRD.md`, which un-declares it.
- **The detail file is a `draft`** — carry on, and **say so plainly in the reply**. Record its
  status and the date in `detail_status` and `detail_read_on`. `docs/process/requirements/README.md`
...
identifier lowercased — `qa/fr-acc-01`. `docs/qa/README.md` says why; this is how.
...
Copy `docs/process/templates/test-scenarios.md` to `docs/qa/scenarios/<area>/<id>.md` — the area is
